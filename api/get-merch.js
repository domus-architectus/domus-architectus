export default async function handler(req, res) {
  // На сервере Vercel CORS-ограничений нет, запрос пройдет мгновенно
  ...
  // Разрешаем твоему же сайту читать данные
  ...
  try {
    ...
    if (!shops || !shops.length) return res.status(404).json({ error: 'Магазин не найден' });
